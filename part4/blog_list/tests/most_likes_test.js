...
describe('returns author with most likes', () => {
  const blogs1 = [
    ...
    test('returns author with the most likes from list', () => {
        const result = listHelper.mostLikes(blogs1)
        assert.deepStrictEqual(result, { author: 'Edsger W. Dijkstra', likes: 12 })
      })
  test ('if blogs is empty, returns empty object', () => {
    const result = listHelper.mostLikes(blogs2)
    assert.deepStrictEqual(result, {})
  })
  test ('if there is a tie, will return an author with equal highest likes', () => {
